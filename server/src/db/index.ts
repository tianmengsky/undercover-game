/**
 * db/index.ts — MySQL + Drizzle ORM 连接
 *
 * 用法:
...
/** 批量更新用户战绩 + exp */
export async function updateUserStats(userId: string, updates: Record<string, unknown>) {
...
/** 人设点赞 */
export async function togglePersonaLike(personaId: string, userId: string): Promise<boolean> {
  const [rows] = await pool.execute('SELECT 1 FROM persona_likes WHERE persona_id = ? AND user_id = ?', [personaId, userId]) as any
  if ((rows as any[]).length > 0) return false
  await pool.execute('INSERT INTO persona_likes (persona_id, user_id) VALUES (?, ?)', [personaId, userId])
  await pool.execute('UPDATE personas SET like_count = like_count + 1 WHERE id = ?', [personaId])
  return true
}

/** 人设使用次数+1 */
export async function incrementPersonaUsage(personaId: string): Promise<void> {
  await pool.execute('UPDATE personas SET usage_count = usage_count + 1 WHERE id = ?', [personaId])
}

/** 月度人设创建限额检查 */
export async function checkPersonaMonthlyLimit(userId: string): Promise<{ allowed: boolean; current: number; max: number }> {
  const now = new Date()
  const month = `${now.getFullYear()}-${now.getMonth() + 1}`
  const [rows] = await pool.execute('SELECT count FROM persona_monthly_limits WHERE user_id = ? AND month = ?', [userId, month]) as any
  const current = (rows as any[])[0]?.count || 0
  return { allowed: current < 3, current, max: 3 }
}

/** 月度人设计数+1 */
export async function incPersonaMonthlyLimit(userId: string): Promise<void> {
  const now = new Date()
  const month = `${now.getFullYear()}-${now.getMonth() + 1}`
  const [rows] = await pool.execute('SELECT count FROM persona_monthly_limits WHERE user_id = ? AND month = ?', [userId, month]) as any
  if ((rows as any[]).length > 0) {
    await pool.execute('UPDATE persona_monthly_limits SET count = count + 1 WHERE user_id = ? AND month = ?', [userId, month])
  } else {
    await pool.execute('INSERT INTO persona_monthly_limits (user_id, month, count) VALUES (?, ?, 1)', [userId, month])
  }
}

/** 月度人设计数-1（删除人设时归还配额） */
export async function decPersonaMonthlyLimit(userId: string): Promise<void> {
  const now = new Date()
  const month = `${now.getFullYear()}-${now.getMonth() + 1}`
  const [rows] = await pool.execute('SELECT count FROM persona_monthly_limits WHERE user_id = ? AND month = ?', [userId, month]) as any
...
const OFFICIAL_PERSONA_SEED: Array<Record<string, unknown>> = [
  {
    ...
    authorId: '_official_', authorName: '官方', usageCount: 0, likeCount: 0, isPublic: true, createdAt: 0,
...
export async function seedDatabase(): Promise<void> {
  // 成就 seed
  const [achRows] = await pool.execute('SELECT count(*) as c FROM achievements') as any
  if (achRows[0].c === 0) {
    for (const a of ACHIEVEMENT_SEED) {
      await pool.execute(
        `INSERT IGNORE INTO achievements (id, name, description, icon, category, \`condition\`, reward_exp) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [a.id, a.name, a.description, a.icon, a.category, '{}', a.rewardExp],
      )
    }
  }
